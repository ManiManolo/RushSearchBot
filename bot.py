...
BACKOFF_MAX = int(os.getenv("BACKOFF_MAX", "600"))  # max 10 minutes
...
# ================== Bot factory =================
def create_bot():
    """Create a fresh bot instance with all handlers registered."""
...
async def main():
    ...
    attempt = 0
    while True:
        bot = create_bot()  # ← NEW instance each attempt
        try:
            log.info("🔐 Starting Discord login...")
            await bot.start(TOKEN)  # internally calls login() then connect()
            break  # running until closed
            ...
            # Always cleanup the bot and its HTTP session
            try:
                await bot.close()
            except Exception:
                pass
